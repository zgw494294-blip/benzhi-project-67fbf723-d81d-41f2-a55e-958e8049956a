...
func (s *Service) mutate(id, key string, expected *int64, fn func(*domain.Trial) error) (*domain.Trial, error) {
	unlock := s.lock(id)
	defer unlock()
	if out, ok := s.Store.LookupIdempotent(id, key); ok {
		return out, nil
	}
...
func (s *Service) LockProtocol(id, key string, p domain.TreatmentProtocol) (*domain.Trial, error) {
	return s.mutate(id, key, nil, func(t *domain.Trial) error { return t.LockProtocol(p) })
}
func (s *Service) LockProtocolChecked(id, key string, expected int64, digest, actor string, p domain.TreatmentProtocol) (*domain.Trial, error) {
	return s.mutate(id, key, &expected, func(t *domain.Trial) error { return t.LockProtocolPreview(p, digest, expected, actor) })
}
func (s *Service) Start(id, key string) (*domain.Trial, error) {
	return s.mutate(id, key, nil, func(t *domain.Trial) error { return t.StartObserving() })
}
func (s *Service) StartChecked(id, key string, expected int64, actor string) (*domain.Trial, error) {
	return s.mutate(id, key, &expected, func(t *domain.Trial) error { return t.StartObservingBy(actor) })
}
func (s *Service) Observe(id, key string, o domain.DailyObservation) (*domain.Trial, error) {
	return s.mutate(id, key, nil, func(t *domain.Trial) error { return t.AddObservation(o) })
}
func (s *Service) ObserveBatch(id, key string, expected int64, day int, observations []domain.DailyObservation, actor string) (*domain.Trial, error) {
	return s.mutate(id, key, &expected, func(t *domain.Trial) error { return t.AddObservationBatch(day, observations, actor) })
}
func (s *Service) Deviation(id, key string, d domain.Deviation) (*domain.Trial, error) {
	return s.mutate(id, key, nil, func(t *domain.Trial) error { return t.AddDeviation(d) })
}
func (s *Service) DeviationChecked(id, key string, expected int64, d domain.Deviation, actor string) (*domain.Trial, error) {
	return s.mutate(id, key, &expected, func(t *domain.Trial) error { return t.AddDeviationBy(d, actor) })
}
func (s *Service) Resolve(id, key, devID string) (*domain.Trial, error) {
	return s.mutate(id, key, nil, func(t *domain.Trial) error { return t.ResolveDeviation(devID) })
}
func (s *Service) ResolveChecked(id, key string, expected int64, devID, responsible, completion string, observationIDs []string, actor string) (*domain.Trial, error) {
	return s.mutate(id, key, &expected, func(t *domain.Trial) error {
...
func (s *Service) Submit(id, key string) (*domain.Trial, error) {
	return s.mutate(id, key, nil, func(t *domain.Trial) error { return t.ReadyForReview() })
}
func (s *Service) SubmitChecked(id, key string, expected int64, actor string) (*domain.Trial, error) {
	return s.mutate(id, key, &expected, func(t *domain.Trial) error { return t.SubmitForReview(actor) })
}
func (s *Service) Review(id, key, decision, reason, reviewer string) (*domain.Trial, error) {
	return s.mutate(id, key, nil, func(t *domain.Trial) error { return t.Review(decision, reason, reviewer) })
}
...
func (s *Service) Correct(id, key string, expected int64, issueID, note string, refs []string, confirm bool, actor string) (*domain.Trial, error) {
	return s.mutate(id, key, &expected, func(t *domain.Trial) error { return t.SubmitCorrection(issueID, note, refs, confirm, actor) })
}
